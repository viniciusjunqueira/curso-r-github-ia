...
Depois do modelo escolhido, defina ele como padrão para ser utilizado no `chattr`.
```{r}
chattr_use(my_chat)
```
...
```{r}
chattr(paste0("Faca uma analise detalhada desse codigo:", pipeline_complexo))
```

#### Caso 2: Gerar uma nova função
...
**Objetivo:** Usar LLM para gerar um arquivo de dados.
...
### Tarefa 3: Criar um gráfico utilizando o `ggplot2`.

**Objetivo:** Usar LLM para automatizar o processo
```{r}
prompt <- 'Crie um gráfico boxplot utilizando o ggplot a partir de dados simulados
           de pesos ao nascimento de bovinos de corte'
chattr(prompt)
